...
def normalize_kalshi_candle_payload(
    ...
    mapping_row = {"ticker": market_ticker, "market_id": market_ticker, "game_id": game_id}
        ...
        rows.append(
            {
                ...
                "volume": _optional_float(candle.get("volume") or candle.get("volume_delta")),
                "open_interest": _optional_float(candle.get("open_interest")),
...
def _history_timestamp(item: dict[str, Any]) -> datetime | None:
    for key in ("end_ts", "ts", "timestamp", "time"):
        value = item.get(key)
        if value is None:
            continue
